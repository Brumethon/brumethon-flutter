...
typedef DecodeJsonObjectFunctionWithGeneric<T, G> = T Function(Map<String, dynamic>, DecodeJsonObjectFunction<G>);
...
class DecodableTools {
  ...
  static T _validateValue<T>(dynamic value) {
    if ( !checkType<T>(value) ) {
      ...
      throw new Exception("wanted a ${T.toString()} type but got a ${value.runtimeType.toString()}");
    }
    return value;
  }
  ...
  static T? _validateNullableField<T>(Map<String, dynamic> dict, String key) {
    ...
    if (value != null && !checkType<T>(value)) {
      ...
      throw new Exception(DEFAULT_PARSE_ERROR(""));
    }
    return value;
  }
  ...
  static List<OutputType> decodeNestedListWithGeneric<OutputType, GenericType>(Map<String, dynamic> jsonObject, String key, DecodeJsonObjectFunctionWithGeneric<OutputType, GenericType> decodeFunctionWithGeneric, DecodeJsonObjectFunction<GenericType> decodeGeneric) {
    List<dynamic> dynamicList = _validateField<List<dynamic>>(jsonObject, key);
  ...
  static List<String> decodeStringList(Map<String, dynamic> jsonObject, String key) {
    ...
    try {
      ...
      throw new Exception("Could not parse $key  , ${error.toString()} ");
    }
  ...
  static DateTime decodeDate(Map<String, dynamic> dict, String key) {
    ...
    if (value == null ||  !checkType<String>(value) ) {
      ...
      throw new Exception(DEFAULT_PARSE_ERROR(key));
    }
...
abstract class Decodable<OutputType, InputType> {
  OutputType decode(InputType toDecode);
}
